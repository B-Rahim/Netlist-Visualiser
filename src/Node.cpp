...
bool Node::fromXml(Net* net, xmlTextReaderPtr reader)
{
    ...
    if (nodeTag == nodeName) {
        ...
        if (not instanceName.empty()) {
            ...
            term->setPosition(instance->getMasterCell()->getSymbol()->getTermPosition(instance->getMasterCell()->getTerm(termName)));
            term->setPosition(term->getPosition().translate(instance->getPosition()));
        } else {
