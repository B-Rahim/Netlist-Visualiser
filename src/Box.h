...
bool Box::intersection(const Box& other) const
{
    ...
    if (((x2_ < other.x1_) or (x1_ > other.x2_))
        and ((y2_ < other.y1_) or (y1_ > other.y2_)))
        return false;
    return true;
}
...
{
    if (not intersection(other))
        return Box();
    return Box(std::max(x1_, other.x1_), std::max(y1_, other.y1_), std::min(x2_, other.x2_), std::min(y2_, other.y2_));
}
